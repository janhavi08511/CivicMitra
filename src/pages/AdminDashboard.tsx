...
<<<<<<< HEAD
        ...
        const comps = completions.map((c: any) => ({ id: c._id, ...c } as Completion));
        setPendingCompletions(comps.filter((c: Completion) => [VerificationStatus.PENDING, VerificationStatus.MANUAL_REVIEW].includes(c.aiVerificationStatus)));
        setStats({
        ...
        const userIds = Array.from(new Set(comps.map((c: Completion) => c.userId)));
        const challengeIds = Array.from(new Set(comps.map((c: Completion) => c.challengeId)));
        ...
        challengeIds.forEach((id) => {
          cMap[id] = { challengeId: id, title: id, description: "", shortDescription: "", category: Category.WATER, difficulty: Difficulty.EASY, points: 10, bonusPointsStreak: 5, iconEmoji: "🌱", bannerImageUrl: "", proofInstructions: "", isDaily: true, isActive: true };
        });
