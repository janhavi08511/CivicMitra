...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Calculate impact from completions with impactData (new)
    const completionImpacts = recentActivity
      .filter(c => c.impactData && c.aiVerificationStatus === VerificationStatus.VERIFIED)
      .map(c => c.impactData!.calculatedImpacts);
    
    // Aggregate all impacts
    const allImpacts = [...completionImpacts, pointsImpact];
    const totalImpact = aggregateImpacts(allImpacts);
    
    // Animate the values
    animateValue(displayCO2, totalImpact.co2_kg_saved, setDisplayCO2);
    animateValue(displayElectricity, totalImpact.electricity_wh_saved, setDisplayElectricity);
